					...
					<div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
						<section className="rounded-xl border border-[#dce7f2] bg-white p-3 shadow-sm">
							<div className="relative h-[520px] overflow-hidden rounded-lg bg-[#e7f0e6] sm:h-[620px]">
								...
								<div className="absolute right-4 top-4 flex flex-col gap-2"><div className="rounded-lg border border-white/80 bg-white/90 p-2 shadow-sm backdrop-blur"><button className="block rounded p-1 text-lg leading-none text-[#5d7891] hover:bg-blue-50">+</button><button className="block rounded p-1 text-lg leading-none text-[#5d7891] hover:bg-blue-50">−</button></div><button onClick={() => setShowHeatmap(!showHeatmap)} aria-label="Toggle civic issue heatmap" className={`rounded-lg border p-2 shadow-sm backdrop-blur transition ${showHeatmap ? "border-orange-300 bg-orange-50 text-orange-600" : "border-white/80 bg-white/90 text-[#5d7891]"}`}><Flame className="h-4 w-4" /></button><button onClick={() => setShowBoundaries(!showBoundaries)} aria-label="Toggle ward and zone boundaries" className={`rounded-lg border p-2 shadow-sm backdrop-blur transition ${showBoundaries ? "border-purple-300 bg-purple-50 text-purple-600" : "border-white/80 bg-white/90 text-[#5d7891]"}`}><Layers className="h-4 w-4" /></button></div>
							</div>
						</section>
